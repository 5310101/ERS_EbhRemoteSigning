...
namespace ESignLibrary
{
    public class ESign
    {
        ...
        public static bool ETAXSignTam(X509Certificate2 card, string InputFile, string OutputFile)
        {
            byte[] sourceArray;
            PdfSignatureAppearance signatureAppearance = null;
            PdfDictionary update = null;
            Dictionary<iTextSharp.text.pdf.PdfName, int> exclusionSizes = new Dictionary<PdfName, int>();
            PdfStamper stamper = null;

            FileStream stream = new FileStream(InputFile, FileMode.Open, FileAccess.ReadWrite);
            FileStream streamOut = null;

            System.Drawing.Image signImage = System.Drawing.Image.FromFile(Application.StartupPath + @"\Config\ImgSign.png");
            iTextSharp.text.Image img = iTextSharp.text.Image.GetInstance(signImage, System.Drawing.Imaging.ImageFormat.Png);
            ...
            try
            {
                ...
                // //streamOut = new FileStream(OutputFile, FileMode.Create); 
                // streamOut = new FileStream(Application.StartupPath + "\\" + "temp", FileMode.Create);
                // PdfReader reader = new PdfReader(buffer);

                // PdfStamper sta = new PdfStamper(reader, stream);
                // PdfContentByte con = sta.GetOverContent(reader.NumberOfPages);

                // float pWidth = reader.GetPageSize(reader.NumberOfPages).Width;
                // float pHeight = reader.GetPageSize(reader.NumberOfPages).Height;
                // //if (pHeight > pWidth && reader.GetPageRotation(reader.NumberOfPages) > 0)
                // //{
                // //    pWidth = reader.GetPageSize(reader.NumberOfPages).Height;
                // //    pHeight = reader.GetPageSize(reader.NumberOfPages).Width;
                // //}
                ...
                //document.SetPageSize(reader.GetPageSize(reader.NumberOfPages));                
                //PdfWriter writer = PdfWriter.GetInstance(document,stream);                
                //document.Open();
                //writer.Open();
                //Paragraph par = new Paragraph("Ký bởi: " + PdfPKCS7.GetSubjectFields(certChain[0]).GetField("CN"));
                ...
                ////signatureAppearance = PdfStamper.CreateSignature(reader, streamOut, '\0').SignatureAppearance;
                //var signField = PdfStamper.CreateSignature(reader, streamOut, '\0', null, true);

                //signatureAppearance = signField.SignatureAppearance;
                //signatureAppearance.SignDate = DateTime.Now;


                //iTextSharp.text.Font font2 = new iTextSharp.text.Font(BaseFont.CreateFont(Application.StartupPath + "\\" + @"times.ttf", "Identity-H", false));
                //font2.Size = 5f;
                ////Get font name from local system
                ...
                //float pWidth = reader.GetPageSize(reader.NumberOfPages).Width;
                //float pHeight = reader.GetPageSize(reader.NumberOfPages).Height;//reader.NumberOfPages
                //if (pHeight > pWidth && reader.GetPageRotation(reader.NumberOfPages) > 0)
                //{
                //    pWidth = reader.GetPageSize(reader.NumberOfPages).Height;
                //    pHeight = reader.GetPageSize(reader.NumberOfPages).Width;
                ...
                //signatureAppearance.SetVisibleSignature(new iTextSharp.text.Rectangle(pWidth, pHeight, pWidth - 200f, pHeight - 20), reader.NumberOfPages, (string)null);
                ////System.Drawing.Image signImage = System.Drawing.Image.FromFile(Application.StartupPath + @"\Config\ImgSign.png");
                //System.Drawing.Image signImage1 = System.Drawing.Image.FromFile(Application.StartupPath + @"\Config\SignBland.png");
                //if (KyGiaLap)
                //{
                //    signatureAppearance.Layer2Text = "Ký khai th?: " + PdfPKCS7.GetSubjectFields(certChain[0]).GetField("CN");
                //}
                //else
                //{
                ...
                //    if (signImage != null)
                //    {
                //        iTextSharp.text.Image i = iTextSharp.text.Image.GetInstance(signImage, BaseColor.WHITE);
                //        i.Alignment = iTextSharp.text.Image.RIGHT_ALIGN;

                //        signatureAppearance.SignatureGraphic = i;
                //        signatureAppearance.SignatureGraphic.Alignment = iTextSharp.text.Image.LEFT_ALIGN;
                //        signatureAppearance.SignatureGraphic.SetAbsolutePosition(0, 50);
                //        signatureAppearance.SignatureGraphic.ScaleAbsoluteHeight(100f);
                //        signatureAppearance.SignatureGraphic.ScaleAbsoluteWidth(100f);
                //        signatureAppearance.Image = iTextSharp.text.Image.GetInstance(signImage1, BaseColor.WHITE); ;
                //        //signatureAppearance.Image.ScaleToFit(0,0);
                ...
                ////var sigAppearence = signatureAppearance.GetAppearance();


                ////TextField field = new TextField(stamp.Writer, new iTextSharp.text.Rectangle(0, 0, 200, 100), "signature" + i.ToString());
                ////field.Text = "Da ky";
                ////stamp.AddAnnotation(field.GetTextField(), i);
                ////stamp.FormFlattening = true; // lock fields and prevent further edits.WINCER_SIGNED
                ...
                ////signature.Date = new PdfDate(signatureAppearance.SignDate);
                //signature.Date = new PdfDate(new DateTime(2010, 1, 1));
                //signature.Name = "";
                ...
                //}
                //rangeStream.Close(); rangeStream.Dispose();
                //algorithm.TransformFinalBlock(buffer2, 0, 0);
                //buffer2 = null;

                ////if (sourceArray == null)
                //sourceArray = SignPass(algorithm.Hash, card, false);
                ...
                //    File.Copy(Application.StartupPath + "\\" + "temp", OutputFile, true);
                //    return true;
                //}
                //reader.Close();
                //stamper.Close();
            ...
            finally
            {
                ...
                if (streamOut != null)
                {
                    streamOut.Close(); streamOut.Dispose();
                }
            }
            //return true;
        }
