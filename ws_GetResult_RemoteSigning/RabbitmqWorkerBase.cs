...
namespace ws_GetResult_RemoteSigning
{
    ...
    {
        protected readonly RabbitmqManager Manager;
        ...
        protected RabbitMqWorkerBase(RabbitmqManager manager,
        ...
        {
            Manager = manager;
            _semaphore = new SemaphoreSlim(concurrentConsumers);
            Options = options;
        ...
        public async Task DeclareInfrastructureAsync()
        {
            ...
            {
                using (var chanel = await Manager.CreateChannelAsync())
                {
        ...
        public async Task StartConsumingAsync(CancellationToken cancellationToken)
        {
            ...
            foreach (var option in _consumerOptions)
            {
                ...
                consumer.ReceivedAsync += async (model, ea) =>
                {
                    ...
                    _ = ProcessMessageInternalAsync(channel, ea, option, cancellationToken);
        ...
        private async Task ProcessMessageInternalAsync(IChannel channel, BasicDeliverEventArgs ea, RabbitMqConsumerOptions option, CancellationToken cancellationToken)
        {
            HoSoMessage hs = null;
            try
            {
                hs = ProcessMessageToObject<HoSoMessage>(ea);
                if (hs == null)
                {
                    throw new Exception("Deserialize error or incorrect message");
                }
                await ProcessMessageAsync(channel, hs, cancellationToken, ea);
            ...
            catch (Exception ex)
            {
                ...
                await HandleFailureAsync(hs, option, channel, ea, ex);
            }
            finally
            {
                _semaphore.Release();
        ...
        public async Task StartConsumingHandleDlqAsync(CancellationToken cancellationToken)
        {
            ...
            foreach (var option in _consumerOptions)
            {
                ...
                {
                    await _semaphore.WaitAsync(cancellationToken);
                    _ = ProcessMessageDlqInternalAsync(channel, ea, option, cancellationToken);
                    await Task.CompletedTask;
                };
        ...
        private async Task ProcessMessageDlqInternalAsync(IChannel channel, BasicDeliverEventArgs ea, RabbitMqConsumerOptions option, CancellationToken cancellationToken)
        {
            HoSoMessage hs = null;
            try
            {
        ...
        protected abstract Task ProcessMessageAsync(IChannel channel, HoSoMessage hs, CancellationToken cancellationToken, BasicDeliverEventArgs ea);
        ...
        /// </summary>
        protected virtual async Task HandleFailureAsync(HoSoMessage hs, RabbitMqConsumerOptions option, IChannel channel, BasicDeliverEventArgs ea, Exception ex)
        {
            ...
            {
                await channel.BasicPublishAsync(option.ExchangeName, option.DeadLetterQueue, false, props, hs.GetBytesStringFromJsonObject()).ConfigureAwait(false);
            }
            else
            {
                await channel.BasicPublishAsync(option.ExchangeName, option.RetryQueue, false, props, hs.GetBytesStringFromJsonObject()).ConfigureAwait(false);
            }
        }


        //truoc khi publish vao retry phai ack message goc, de ko bi block queue
        protected async Task PublishToRetryQueueAsync(IChannel channel, string queueName, string dlqName, byte[] body, int retryCount, int maxRetry, string lastError = null)
        {
            var props = new BasicProperties
            {
                DeliveryMode = DeliveryModes.Persistent,
        ...
        protected async Task PublishToAnotherQueue(IChannel channel, string exchangeName, string queueName, byte[] body)
        {
            var props = new BasicProperties
            {
                DeliveryMode = DeliveryModes.Persistent,
